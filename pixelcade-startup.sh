...
# check to see if there are any startup scripts and call them else continue here
if [ "$(ls -A /home/pi/.emulationstation/scripts/pixelcade-start)" ]; then
        for each in /home/pi/.emulationstation/scripts/pixelcade-start/*.sh ; do bash $each; done
else
